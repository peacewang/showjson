...
ShowJSON 是一个轻量、本地优先的 macOS 与 Windows JSON 快速查看器。

在浏览器、终端或任意应用中复制文本，按下全局快捷键，即可立即查看结构化结果，不必再把内容粘贴到 IDE 或在线格式化网站。默认快捷键是 `Cmd/Ctrl + Shift + J`，可在托盘菜单的 `Settings…` 中修改。
...
3. 按 `Cmd + Shift + J`（macOS）或 `Ctrl + Shift + J`（Windows）。
4. 在 ShowJSON 中查看、搜索或复制结果。
5. 按 `Esc`、关闭窗口或切换到其他应用即可隐藏窗口。

只有全局快捷键会读取并识别剪贴板。点击应用图标或托盘图标只会显示现有界面，不会读取新的剪贴板内容。

需要直接读取其他应用中的选中文本时，可在托盘菜单 `Settings…` 中开启“优先读取选中文本”。macOS 会要求授予“辅助功能”权限；不授权时 ShowJSON 仍可按原方式读取剪贴板。
